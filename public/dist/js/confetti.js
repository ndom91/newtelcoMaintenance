...
function createElements(btn, elementCount, colors, width, height) {
...
function animate(btn, fettis, dragFriction, decay, duration, delay) {
  let startTime;
    ...
    function update(time) {
      ...
      if (time - startTime < duration) {
        ...
        fettis.forEach(fetti => {
          if (fetti.element.parentNode === btn) {
            return btn.removeChild(fetti.element);
          }
        });
        resolve();
      }
...
function confetti(btn, config = {}) {
  ...
  const elements = createElements(btn, elementCount, colors, width, height);
  const fettis = elements.map(element => ({
    element,
    physics: randomPhysics(angle, spread, startVelocity, random)
  }));

  return animate(btn, fettis, dragFriction, decay, duration, delay);
}